import getCompareIsAllincartVersion from './compare-version';

type Comparator = '=' | '!=' | '<' | '>' | '<=' | '>=';

describe('getCompareAllincartVersion', () => {
  const mock = (version: string) => () => Promise.resolve(version);

  describe('It works with allincart versions and semver versions', () => {
    it.each<[boolean, string, Comparator, string]>([
      ...
      [false, '6.8.2', '=', '6.6.9.2'],
      [false, '6.8.2', '=', '6.9.2'],
    ])("returns %s for '%s %s %s'", async (exptectedResult, allincartVersion, comparator, comparedVersion) => {
      const compareIsAllincartVersion = getCompareIsAllincartVersion(mock(allincartVersion));

      expect(await compareIsAllincartVersion(comparator, comparedVersion)).toBe(exptectedResult)
    });
  });
