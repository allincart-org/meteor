...
import type { AllincartMessageTypes } from './message-types';
...
/**
 * Resembles the options that are available on any AllincartMessageType.
 */
 ...
 * @internal
 */
export type MessageDataType<TYPE extends keyof AllincartMessageTypes> = Omit<AllincartMessageTypes[TYPE], 'responseType'>;

/**
 * This is the structure of the data which will be send with {@link send}
 * @internal
 */
export type AllincartMessageSendData<MESSAGE_TYPE extends keyof AllincartMessageTypes> = {
  _type: MESSAGE_TYPE,
 ...
 * @internal
 */
export type AllincartMessageResponseData<MESSAGE_TYPE extends keyof AllincartMessageTypes> = {
  _type: MESSAGE_TYPE,
  _response: AllincartMessageTypes[MESSAGE_TYPE]['responseType'] | null,
  _callbackId: string,
}
 ...
 */
export function send<MESSAGE_TYPE extends keyof AllincartMessageTypes>(
  type: MESSAGE_TYPE,
  data: MessageDataType<MESSAGE_TYPE> & BaseMessageOptions,
  _targetWindow?: Window,
  _origin?: string
): Promise<AllincartMessageTypes[MESSAGE_TYPE]['responseType'] | null> {
  ...
  const messageData: AllincartMessageSendData<MESSAGE_TYPE> = {
    _type: type,
    _data: sendData,
  ...
  let serializedData = serialize(messageData) as AllincartMessageSendData<MESSAGE_TYPE>;
  ...
  if (_origin) {
    ...
    if (validationErrors) {
      ...
      if ([
        ...
        }) as AllincartMessageSendData<MESSAGE_TYPE>;
      }
  ...
  return new Promise((resolve, reject) => {
    const callbackHandler = function(event: MessageEvent<string>):void {
      ...
      let allincartResponseData;
      // Try to parse the json response
      try {
        allincartResponseData = JSON.parse(event.data) as unknown;
      } catch {
      ...
      if (!isMessageResponseData<MESSAGE_TYPE>(allincartResponseData)) {
        return;
      }

      // Only execute if response value exists
      if (!allincartResponseData.hasOwnProperty('_response')) {
        return;
      }

      // Deserialize methods etc. so that they are callable in JS
      const deserializedResponseData = deserialize(allincartResponseData, event) as AllincartMessageResponseData<MESSAGE_TYPE>;
...
export type HandleMethod<MESSAGE_TYPE extends keyof AllincartMessageTypes> = (
  data: MessageDataType<MESSAGE_TYPE> & BaseMessageOptions,
  additionalInformation: { _event_: MessageEvent<string> }
) => Promise<AllincartMessageTypes[MESSAGE_TYPE]['responseType']> | AllincartMessageTypes[MESSAGE_TYPE]['responseType'];
 ...
 */
export function handle<MESSAGE_TYPE extends keyof AllincartMessageTypes>
(
  type: MESSAGE_TYPE,
  ...
  : () => void
{
  const handleListener = async function(event: MessageEvent<string>): Promise<void> {
    ...
    let allincartMessageData;

    // Try to parse the json file
    try {
      allincartMessageData = JSON.parse(event.data) as unknown;
    } catch {
      // Fail silently when message is not a valid json file
      return;
    }
    ...
    // Check if messageData is valid
    if (!isMessageData<MESSAGE_TYPE>(allincartMessageData)) {
      return;
    }

    // Deserialize methods etc. so that they are callable in JS
    const deserializedMessageData = deserialize(allincartMessageData, event) as AllincartMessageSendData<MESSAGE_TYPE>;
    ...
    const responseValue = await Promise.resolve((() => {
       ...
       */
      const validationErrors = validate({
        serializedData: allincartMessageData,
        origin: event.origin,
        type: type,
    ...
    const responseMessage: AllincartMessageResponseData<MESSAGE_TYPE> = {
    ...
    const serializedResponseMessage = ((): AllincartMessageResponseData<MESSAGE_TYPE> => {
      let serializedMessage = serialize(responseMessage) as AllincartMessageResponseData<MESSAGE_TYPE>;
      ...
      if (validationErrors) {
        ...
        serializedMessage = serialize(serializedMessage) as AllincartMessageResponseData<MESSAGE_TYPE>;
      }
...
export function publish<MESSAGE_TYPE extends keyof AllincartMessageTypes>(
  type: MESSAGE_TYPE,
  data: AllincartMessageTypes[MESSAGE_TYPE]['responseType'],
  sources: {
    source: Window,
    origin: string,
    sdkVersion: string | undefined,
...
export function subscribe<MESSAGE_TYPE extends keyof AllincartMessageTypes>(
  type: MESSAGE_TYPE,
  method: (data: AllincartMessageTypes[MESSAGE_TYPE]['responseType']) => void | Promise<unknown>
): () => void {
...
export function createSender<MESSAGE_TYPE extends keyof AllincartMessageTypes>
(messageType: MESSAGE_TYPE, baseMessageOptions: MessageDataType<MESSAGE_TYPE>)
:(messageOptions?: MessageDataType<MESSAGE_TYPE> & BaseMessageOptions) => Promise<AllincartMessageTypes[MESSAGE_TYPE]['responseType']>

// SENDER WITH PARTIAL ARGUMENTS (ARGUMENTS DEFINED IN BASE OPTIONS ARE OMITTED)
export function createSender<MESSAGE_TYPE extends keyof AllincartMessageTypes, BASE_OPTIONS extends Partial<MessageDataType<MESSAGE_TYPE>>>
(messageType: MESSAGE_TYPE, baseMessageOptions: BASE_OPTIONS)
:(messageOptions: Omit<MessageDataType<MESSAGE_TYPE>, keyof BASE_OPTIONS> & BaseMessageOptions) => Promise<AllincartMessageTypes[MESSAGE_TYPE]['responseType']>
 ...
 */
export function createHandler<MESSAGE_TYPE extends keyof AllincartMessageTypes>(messageType: MESSAGE_TYPE) {
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
 ...
 * defined and can be hidden.
 */
export function createSubscriber<MESSAGE_TYPE extends keyof AllincartMessageTypes>(messageType: MESSAGE_TYPE) {
  // eslint-disable-next-line @typescript-eslint/explicit-function-return-type
  return (method: (data: AllincartMessageTypes[MESSAGE_TYPE]['responseType']) => void | Promise<unknown>, id?: string) => {
    if (!id) {
      return subscribe(messageType, method);
    }

    const wrapper = (data: AllincartMessageTypes[MESSAGE_TYPE]['responseType']): void => {
      if (data.id === id) {
        void method(data);
      }
    };
 ...
 */
function isMessageData<MESSAGE_TYPE extends keyof AllincartMessageTypes>(eventData: unknown): eventData is AllincartMessageSendData<MESSAGE_TYPE> {
  const allincartMessageData = eventData as AllincartMessageSendData<MESSAGE_TYPE>;

  return !!allincartMessageData._type
         && !!allincartMessageData._data
         && !!allincartMessageData._callbackId;
}

// AllincartMessageTypes[MESSAGE_TYPE]['responseType']
function isMessageResponseData<MESSAGE_TYPE extends keyof AllincartMessageTypes>(eventData: unknown): eventData is AllincartMessageResponseData<MESSAGE_TYPE> {
  const allincartMessageData = eventData as AllincartMessageResponseData<MESSAGE_TYPE>;

  return !!allincartMessageData._type
         && !!allincartMessageData.hasOwnProperty('_response')
         && !!allincartMessageData._callbackId;
}
