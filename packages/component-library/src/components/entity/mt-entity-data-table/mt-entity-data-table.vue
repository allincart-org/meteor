 ...
 */
import type Repository from "@allincart-ag/meteor-admin-sdk/es/data/repository";
import Criteria from "@allincart-ag/meteor-admin-sdk/es/data/Criteria";
import { useRepository } from "@allincart-ag/meteor-admin-sdk/es/data/composables/useRepository";
import { getRepository } from "@allincart-ag/meteor-admin-sdk/es/data/composables/getRepository";
import MtDataTable, {
  type ColumnChanges,
...
const handleItemDelete = async ({ id }: { id: string }) => {
  ...
  if (isInIframe && !props.forceRealModal) {
    const { ui } = await import("@allincart-ag/meteor-admin-sdk");
